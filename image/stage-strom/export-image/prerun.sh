...
# Keep each partition 8 MiB aligned, matching pi-gen's default export layout.
ALIGN="$((8 * 1024 * 1024))"
ROOT_MARGIN="$(echo "($ROOT_SIZE * 0.2 + 200 * 1024 * 1024) / 1" | bc)"
STATE_PART_SIZE="$((STROM_STATE_PART_SIZE_MB * 1024 * 1024))"

BOOT_PART_START=$((ALIGN))
BOOT_PART_SIZE=$(((BOOT_SIZE + ALIGN - 1) / ALIGN * ALIGN))
ROOT_PART_START=$((BOOT_PART_START + BOOT_PART_SIZE))
ROOT_PART_SIZE=$(((ROOT_SIZE + ROOT_MARGIN + ALIGN - 1) / ALIGN * ALIGN))
STATE_PART_START=$((ROOT_PART_START + ROOT_PART_SIZE))
STATE_PART_SIZE=$(((STATE_PART_SIZE + ALIGN - 1) / ALIGN * ALIGN))
IMG_SIZE=$((STATE_PART_START + STATE_PART_SIZE))
...
mkfs.ext4 -L strom-state -O "$ROOT_FEATURES" "$STATE_DEV" > /dev/null
