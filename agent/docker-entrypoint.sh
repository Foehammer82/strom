...
mkdir -p /etc/strom /var/lib/strom /etc/nut "$SIM_DIR"

if [ ! -f /etc/strom/agent.yaml ]; then
    cat >/etc/strom/agent.yaml <<EOF
nut:
  username: ${NUT_USER}
  password: ${NUT_PASS}
EOF
...
set -- /app/strom-agent --config-dir /etc/nut --listen :80 --tls-listen :8443 --simulate "$SIM_DIR" --node-id "$NODE_ID"
